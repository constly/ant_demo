...
// ..\..\..\..\..\bin\msvc\Release\shaderc.exe --platform windows --type fragment -p s_5_0 -f mesh\fs_mesh.sc -o bin\windows\direct3d11\fs_mesh.bin --depends -i ..\..\..\..\..\..\ant\pkg\ant.resources\shaders --debug
...
uniform SamplerState s_texSampler : register( s[0] ); uniform Texture2D s_texTexture : register( t[0] ); static BgfxSampler2D s_tex = { s_texSampler, s_texTexture };
void main( float4 gl_FragCoord : SV_POSITION , out float4 bgfx_FragData0 : SV_TARGET0 )
{
float4 bgfx_VoidFrag = vec4_splat(0.0);
bgfx_FragData0 = bgfxTexture2D(s_tex, float2(0, 0));
}
