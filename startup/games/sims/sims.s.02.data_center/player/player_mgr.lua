...
local function new(data_center)
	...
	function api.load_from_save(data)
		...
		for i, p in ipairs(data.players or {}) do 
			...
			local player = new_player_handler.new(data_center, p.id)
			player.guid = p.guid
			player.name = p.name
			...
			player.npc = data_center.npc_mgr.get_npc_by_id(p.npc_id) 
			if not player.npc then 
				player.npc = data_center.npc_mgr.create_player_npc(player)
			end
			...
			data_center.main_world.on_login(player, login_param)
	...
	function api.add_player(fd, guid)
		...
		next_id = next_id + 1;
		local player = new_player_handler.new(data_center, next_id)
		player.fd = fd
		player.guid = guid
		player.npc = data_center.npc_mgr.create_player_npc(player) 
		table.insert(api.players, player)
